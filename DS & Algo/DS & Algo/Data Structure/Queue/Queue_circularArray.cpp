...
{
    return ((Q->rear_pointer+1)%Q->size_q==Q->front_pointer);
}
...
void enQueue(struct Queue *Q,int x)
{
    if(isFull(Q)){
        cout<<"Queue is Full\n";
        return;}
        Q->rear_pointer=(Q->rear_pointer+1)%Q->size_q;
...
int deQueue(struct Queue *Q)
{
    ...
    {
        Q->front_pointer=(Q->front_pointer+1)%Q->size_q;
...
void display(struct Queue *Q)
{
    ...
    int i=(Q->front_pointer);
    while(i != Q->rear_pointer)
    {
        i=(i+1)%Q->size_q;
        cout<<Q->arr[i]<<" ";
    }
